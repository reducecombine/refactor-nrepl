...
(defn namespace-aliases
  ...
  ([ignore-errors?]
   {:clj  (->> (core/find-in-project (util/wrap-ignore-errors (every-pred core/readable-file?
                                                                          (some-fn core/clj-file? core/cljc-file?))
                                                              ignore-errors?))
               (map (partial get-libspec-from-file-with-caching :clj))
               aliases-by-frequencies)
    :cljs (->> (core/find-in-project (util/wrap-ignore-errors (every-pred core/readable-file?
